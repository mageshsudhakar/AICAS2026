...
module fp_divider_top (
    ...
    // --------------------------------------------------------
    // 2. Tile Indexing & High-Precision Deltas
    // --------------------------------------------------------
    // Tiles are 8x8 (3 bits each). dx/dy use the remaining 20 bits.
    wire [2:0] h = mx[22:20];
    wire [2:0] k = my[22:20];
    wire [19:0] dx = mx[19:0];
    wire [19:0] dy = my[19:0];

    // --------------------------------------------------------
    // 3. High-Resolution 64-Tile LUT
    // --------------------------------------------------------
    // A, B: 10-bit signed (LSB = 2^-7)
    // C: 12-bit unsigned (LSB = 2^-10)
    reg signed [9:0] A, B; 
    reg [11:0] C; 
    ...
    // --------------------------------------------------------
    // 4. Fixed-Point Arithmetic (Scaled to 2^-30)
    // --------------------------------------------------------
    // Aligning based on LSBs: A,B use 2^-7, dx,dy use 2^-23 -> Product is 2^-30.
    // C uses 2^-10, so shift left by 20 to align with 2^-30.
    wire signed [40:0] term_a = $signed({1'b0, dx}) * A; 
    wire signed [40:0] term_b = $signed({1'b0, dy}) * B;
    wire signed [40:0] term_c = $signed({1'b0, C}) << 20; 

    wire signed [40:0] P_sum = term_a + term_b + term_c;

    // --------------------------------------------------------
    // 5. Normalization (Threshold 1.0 = 2^30)
    // --------------------------------------------------------
    localparam signed [40:0] ONE_VAL = 41'd1073741824; 
    ...
    always @(*) begin
        if (P_sum < ONE_VAL) begin
            // Result is in [0.5, 1.0) -> requires shift left, exp -1
            q_mant  = P_sum[28:6]; 
            exp_adj = -9'sd1;
        end else begin
            // Result is in [1.0, 2.0) -> normal range
            q_mant  = P_sum[29:7];
            exp_adj = 9'sd0;
        end
    end
    ...
    wire div_by_zero = (Y[30:0] == 0);

    assign Q = div_by_zero ? 32'h7F800000 : {sx ^ sy, ez, q_mant};
